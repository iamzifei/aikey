...
description: |
  ...
  触发方式：/aikey-cut、/剪片、/aikey-剪、「把这条口播剪出来」「口播素材剪成成片」「用 aikey-cut 剪」。普通的「加字幕」「剪一下」不自动触发，先问是不是要走这套口播剪辑流程
...
> **它管的是「拍完之后」。** 拍之前的选题、写稿、审核归 `/aikey-topic` `/aikey-script` `/aikey-review`。
>
...
3. 读 `aikey/references/家族公约.md`（读不到 → 明说「公约读不到，红线无法保证」并停下）
   + `aikey/references/交互规范.md`（🔴 **不是读一遍就算**：收尾按 §四 三件套 —— Recap · Before/After · **下一步给编号选项**；缺信息按 §四 用**选择题**问，**一次只问一个**；不适用的情况见 §五）
4. 读 `{vault}/00-规则与索引/口播拍摄基线.md`（时间/地点/穿着一次定死）
5. 读记忆 `{vault}/08-技能记忆/aikey-cut/` + `_通用/`
...
> 🟢 澳洲AI教父 2026-08-27 定。**这些是默认值，不是每次都要确认的问题。**
> 用户没有说要改，就直接用；用户说了别的，以用户当次说的为准。
>
> ⚠️ **下表是「这一个账号」的实测值，不是通用推荐。**
> 有 `config` 就**从 `config` 读**；读不到就**先问是不是澳洲AI教父账号**：是才用下表，不是就逐项问，不套用。
...
写入 `{vault}/08-技能记忆/aikey-cut/`，**先查重**。
