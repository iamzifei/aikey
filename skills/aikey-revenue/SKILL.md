...
# aikey-revenue：营收异动归因

先读 `config.yaml`（读不到 → 明说配置缺失并停下，不用示例值假装是用户的设定），再读 `aikey/references/交互规范.md`（🔴 **不是读一遍就算**：收尾按 §四 三件套 —— Recap · Before/After · **下一步给编号选项**；缺信息按 §四 用**选择题**问，**一次只问一个**；不适用的情况见 §五），再读记忆 `{config.paths.memory}/aikey-revenue/` + `_通用/`。
...
- **不把一次性收入当新水位**（公理 6）。
- **不做增长方案**（那是另一件事）；不做客户集中度处理（→ `/aikey-concentration`）。
- **不编数字填表**。缺就空着标「无数据」。
...
写入 `{config.paths.memory}/aikey-revenue/`，先查重。
