...
# aikey-skillify：把跑通的做法固化成技能

先读 `config.yaml`（读不到 → 明说配置缺失并停下，不用示例值假装是用户的设定），再读 `aikey/references/交互规范.md`（🔴 **不是读一遍就算**：收尾按 §四 三件套 —— Recap · Before/After · **下一步给编号选项**；缺信息按 §四 用**选择题**问，**一次只问一个**；不适用的情况见 §五），再读 `aikey/references/家族公约.md`，再读记忆 `{config.paths.memory}/aikey-skillify/` + `_通用/`。
...
🔴 **最后一条最容易漏。** 先跑 `bash aikey/scripts/list-skills.sh` 看现有的，明确说出「它和 `aikey-xxx` 的边界是什么」。说不清 → 不该新建。
...
现成的例子：`aikey-flow/evals/`（重设计前后各跑一版，v0.1.0 与 v0.2.0 结果都在）、`aikey-resonate/evals/`、`aikey-review/evals/`（2026-09-06 建）。
...
顺手写一条记忆到 `{config.paths.memory}/aikey-skillify/`：这次固化的是什么、门禁是怎么过的、有没有踩到重叠。**先查重**。
